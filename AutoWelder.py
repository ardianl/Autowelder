    ...
    def __init__(self, settings):
        ...
        self.tab0 = [      
            ...
            [sg.CBox('Fixture Point',    key='Fixture',      default=self.config['Fixture']),
             sg.CBox('Show Fixture Area',    key='FixtureBox',      default=self.config['FixtureVisible']),
             sg.CBox('Show Fixture Features',    key='FixtureVisible',      default=self.config['FixtureVisible'])],
            [sg.Frame('Offset',[
            ])],
            [sg.Frame('Fixture',[
            ...
            [sg.Text('Size',size=(4,1)),
             ...
             sg.Slider((0, 100),self.config['cMax'],1,orientation='h',size=(20, 10),key='cMax')],
        ...
        self.tab2 = [
            ...
            [sg.Text('Image Select',size=(9,1)), sg.Slider((1, 10),1,1,orientation='h',size=(40, 15),key='imsave',)],
        ...
        self.col = [
            ...
            [sg.Button('Select Front Edge'),sg.Button('Select Fixture')],
            [sg.Button('Save and Exit'),sg.Button('Capture Image')],
            [sg.TabGroup([[sg.Tab('Electrode',self.tab0), sg.Tab('Operation', self.tab1),sg.Tab('Debugging',self.tab2)]])],#, sg.Tab('TESTING', self.tab3)
        ]
...
class Frame:   
    ...
    def find_gap(self,array,Fixture,FixtureBox,FixtureVisible,h,s,v,H,S,V,fx1,fx2,fy1,fy2,x1,x2,y1,y2,xoff,yoff,cmin,cmax):
        ...
        if Fixture:
            ...
            for contour in contours:
                if FixtureVisible:cv2.drawContours(self.overlay[fy1:fy2,fx1:fx2], contours, -1, (36,255,12), 2)
...
class Camera: #INITIALIZE SELECTED CAMERA, SET CAMERA PROPERTIES (STREAM BUFFER, FPS, EXPOSURE, GAIN)
    ...
    def get_image(self, Rotate):#RETRIEVE LATEST IMAGE FROM CAMERA
        if Rotate: return np.rot90(self.cam.GetNextImage().GetData().reshape(self.height,self.width,1),3)
        else:      return self.cam.GetNextImage().GetData().reshape(self.height,self.width,1)
...
def startup():#Checks for connected cameras and settings file, loads preexisting settings for known cameras and returns settings dictionary
    welders = {'20348005':'PW1','20347994':'PW3'}
    ...
    for i in range(cam_list.GetSize()):
         ...
         print(cam_list.GetByIndex(i).IsStreaming())
    ...
    if N_cams == 1:
        ...
        if os.path.isfile('{}Psave.txt'.format(cams['cam0'])):   settings = pickle.load(open('{}Psave.txt'.format(cams['cam0']),'rb'))
        else                                                 :   v = cam_setup(list(cams.values())[0], welders[list(cams.values())[0]])
        settings.update(v)
        window.close()
    if N_cams == 2:# or not os.path.isfile('{}Psave.txt'.format(cams['cam0'])):
        print('Multiple cameras detected...')        
        options = [v for k, v in welders.items() if k in list(cams.values())]
        config_layout = [
        ...
        if os.path.isfile('{}Psave.txt'.format(selected_cam)):  settings = pickle.load(open('{}Psave.txt'.format(selected_cam),'rb'))
        else                                                 :  v = cam_setup(selected_cam, selected_welder)
