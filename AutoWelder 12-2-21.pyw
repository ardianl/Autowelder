...
class Instance: #Creates pyGUI window for each camera instance
    def __init__(self, settings):
        ...
        self.tab0 = [
            ...
            [sg.CBox('Reverse Offset',          size=(35,1), key='REVERSE',       default=False)],
    ...
    def loop(self):
        ...
        try:
            sqx, sqy = self.image.find_gap(self.image.blur(self.image.img,self.values['BLUR VAL'], (int(2*self.values['KERNEL']+1))),
                ...
                self.values['SQUARE X'],self.values['SQUARE Y'],self.values['REVERSE'],
...
class Frame:   
    ...
    def find_gap(self,array,Fixture,FixtureBox,FixtureVisible,h,s,v,H,S,V,fx1,fx2,fy1,fy2,x1,x2,y1,y2,xoff,yoff,reverse,cmin,cmax):
        if reverse:
            xoff *= -1
            yoff *= -1
